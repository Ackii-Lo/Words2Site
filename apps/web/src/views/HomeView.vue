...
const waitingText = computed(() => {
  ...
  if (!s) return "排队中…";
  if (s.status === "queued")
    return s.queuePosition > 0
      ? `前面还有 ${s.queuePosition} 位`
      : "马上就到你了…";
  ...
  <div class="mx-auto min-h-dvh w-full max-w-lg px-4 pb-16 pt-10">
    ...
    <template v-if="step === 'intro'">
      <Card class="space-y-5 p-6">
        ...
        <Button size="xl" class="w-full" @click="step = 'record'"
          >开始体验</Button
        >
      </Card>
    </template>
    ...
    <template v-else-if="step === 'confirm'">
      <Card class="space-y-4 p-6">
        ...
        <p
          ...
          :class="
            draft.length > 300 || draft.trim().length < 10
              ? 'text-destructive'
        ...
        <Button variant="ghost" class="w-full" @click="restart"
          ><RotateCcw class="h-4 w-4" /> 重新说</Button
        >
      </Card>
    </template>
    ...
    <template v-else-if="step === 'info'">
      <Card class="space-y-5 p-6">
        <h2 class="flex items-center gap-2 font-semibold">
          <AtSign class="h-4 w-4" /> 填写邮箱，给自己的网页选个网址
        </h2>
        ...
        <div>
          <label class="mb-1 block text-sm text-muted-foreground"
            >网址（只能用小写字母、数字、连字符）</label
          >
          <div
            class="flex items-stretch overflow-hidden rounded-lg border bg-card focus-within:ring-2 focus-within:ring-ring"
          >
            <input
              v-model="domainLabel"
              placeholder="my-cat"
            ...
            />
            <span
              class="flex items-center bg-muted px-3 font-mono text-sm text-muted-foreground"
              >{{ domainSuffix }}</span
            >
          </div>
        ...
        <label
          ...
          :class="isPublic ? 'border-primary/40 bg-primary/5' : ''"
        >
          <input
            v-model="isPublic"
            type="checkbox"
            class="mt-1 h-5 w-5 accent-[var(--primary)]"
          />
          <span>
            <span class="flex items-center gap-1.5 text-sm font-medium"
              ><Eye class="h-4 w-4" /> 上大屏展示</span
            >
        ...
        <Button variant="ghost" class="w-full" @click="step = 'confirm'"
          ><RotateCcw class="h-4 w-4" /> 返回修改描述</Button
        >
      </Card>
    </template>
    ...
    <template v-else-if="step === 'waiting'">
      <Card class="space-y-6 p-8 text-center">
        <div
          class="mx-auto flex h-24 w-24 animate-pulse items-center justify-center rounded-full bg-primary/10"
        >
        ...
        <div>
          ...
          <div class="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-muted">
            <div
              class="h-full w-1/3 animate-[slide_1.5s_ease-in-out_infinite] rounded-full bg-primary"
            ></div>
          </div>
          <div class="mt-3 text-sm text-muted-foreground">
            已等待 {{ waitElapsed }} 秒，一般 1–3 分钟
          </div>
        </div>
        <div class="rounded-xl bg-muted/60 p-4 text-sm text-muted-foreground">
          {{ funFact }}
        </div>
    ...
    <template v-else-if="step === 'preview'">
      <div class="space-y-4">
        ...
        <p v-if="submitError" class="text-center text-sm text-destructive">
          {{ submitError }}
        </p>
        <Button
          size="xl"
        ...
        >
          <div class="flex items-center gap-2 text-sm font-medium">
            <PenLine class="h-4 w-4" /> 想改改？告诉 AI 哪里不满意
          </div>
          ...
          <p class="text-xs text-muted-foreground">
            剩余修改次数：{{
              (status?.maxRefine ?? 2) - (status?.refinements ?? 0)
            }}
            次
            <span v-if="refineError" class="text-destructive">
              · {{ refineError }}</span
            >
          </p>
        </Card>
        ...
        <Button variant="ghost" class="w-full" @click="restart"
          ><RefreshCw class="h-4 w-4" /> 完全重新来一个</Button
        >
      </div>
    </template>
    ...
    <template v-else-if="step === 'certificate' && cert">
      <CertificateCard
        ...
        :email="cert.email"
      />
      <Button variant="ghost" class="mt-4 w-full" @click="restart"
        ><Plus class="h-4 w-4" /> 帮朋友也做一个</Button
      >
    </template>

    <footer class="mt-12 text-center text-xs text-muted-foreground">
      Words to Website · Activity 3
    </footer>
  </div>
</template>
