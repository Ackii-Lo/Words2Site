  ...
  <div class="space-y-5">
    <div class="flex flex-col items-center gap-4 py-6">
      ...
      <div v-else class="flex flex-col items-center gap-4">
        <div
          class="relative flex h-32 w-32 items-center justify-center rounded-full bg-destructive text-white shadow-lg"
        >
          <span
            class="absolute inset-0 animate-ping rounded-full bg-destructive/30"
          ></span>
          <div class="text-center">
            <Mic class="mx-auto h-12 w-12" />
            <div class="mt-1 text-sm">
              录音中 {{ rec.elapsed.value }}s / 90s
            </div>
          </div>
        </div>
    ...
    <p v-if="rec.message.value" class="text-center text-sm text-amber-600">
      {{ rec.message.value }}，请打字输入
    </p>
    <p v-if="uploadError" class="text-center text-sm text-destructive">
      {{ uploadError }}，请打字输入
    </p>
